...
class EditRestaurantPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    descController.text = restaurantData['description'] ?? "";
    return SafeArea(
        child: BlocConsumer<AddRestaurantCubit, AddRestaurantState>(
          listener: (context, state) {
            if(state.status == AddRestaurantStatus.updateSuccess){
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Center(
                          child: Text("Edit Restaurant",style: TextStyle(
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                        ...
                        ? null
                            : (){
                          context.read<AddRestaurantCubit>().updateRestaurant(
                              restarantId: restaurantData['id'],
                              name: nameController.text,
                              description: descController.text,
                              restaurantStatus: state.restaurantStatus ?? restaurantData['status'],
                          );
                        },

                        child: state.status == AddRestaurantStatus.loading
                       ? Center(child: CircularProgressIndicator(color: AppColors.primary,),)
                            : Text("Update Restaurant", style: TextStyle(color: Colors.white)),
                    ),
                  ),
